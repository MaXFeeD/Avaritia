...
function addNamedCallback(action, current) {
	Callback.addCallback(current, function() {
		...
		(arguments.shift(current), action.apply(arguments));
	});
}

function makeSimplifiedCallback(action) {
	var arguments = Array.prototype.slice(arguments);
	if (arguments.length == 0) return Logger.Log("cant create makeSimplifiedCallback without action and names", "ERROR");
	if (arguments.length == 1) return Logger.Log("no names specified for makeSimplifiedCallback", "ERROR");
	arguments.unshift(); // removing action from callback names, it attached into function
	arguments.forEach(function(current, index) { addNamedCallback(action, current); });
}
